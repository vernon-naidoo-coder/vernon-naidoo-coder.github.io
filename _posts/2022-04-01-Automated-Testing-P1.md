...
*You don't **need** to read this bit, but it does give you a good idea of what Automated Testing is, why we do it, the benefits of Automated Testing and all that other good stuff. But as I said, you don't **have** to read it - you can just skip to [Part 2](/_posts/2022-04-01-Automated-Testing-P2.md) of the series...* 
...
Any successful development cycle will require the execution of the same test suite repeatedly. By using a test automation tool, it’s possible to 'record' this test suite and re-play it as required. Once the test suite is automated, no human is required - so make sure about your union rules. This improves the ROI of Test Automation. The ultimate goal of Test Automation is to reduce the number of test cases to be run manually, limit mistakes (by tired (or hungover) humans) and **not** to eliminate Manual Testing.
...
I'm not going to go into lengthy details about each - that's why Google is your best friend - and besides, this post is getting pretty long already.
...
This is where the parts of the application to be tested are defined. Sometimes there isn't enough time (or money) to automate everything at once! Consider the following points when deciding the scope of work:
...
4. How feasible is it to automate a test? - complex tests require too much effort = too much time = too much money
...
1. What tool(s) are we going to use?
2. What features do these tools have?
...
Automated Testing is a software testing technique that allows tests to be run without human intervention. It increases the effectiveness, quality and speed of testing. It is important to define the scope of testing and the definition of *done*. Constant maintenance will be required to ensure that any new features are tested as they are released.
